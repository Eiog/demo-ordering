		...
		<view class="relative overflow" :class="[`round-${props.round} mx-${props.margin[0]} my-${props.margin[1]}`]" :style="[
			...
			<swiper :indicator-dots="false" :interval="props.interval" :duration="props.duration"
				...
				<swiper-item v-for="(item, index) in _list" :key="index" :style="[
					...
					<tm-image v-if="item.type == listItemType.img" :src="item.url" :width="props.width" model="aspectFill"
						:height="props.height"></tm-image>
					<tm-image v-if="item.type == listItemType.video && item.img && _current != index" :src="item.img" model="aspectFill"
